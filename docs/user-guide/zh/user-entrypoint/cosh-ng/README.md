...
- 在另一个 Terminal 把 `gateway_socket` 设为相同绝对路径，再把 intent 传给 Task API：
  ...
  Managed Run surface 把相同 coordinator decision 暴露为 `start`、`answer`、`cancel`、
  `retry` 和 `resolve-approval`，并增加 evidence-aware 的 `inspect` 投影。
- [Managed Run](managed-run.md)：查看稳定的 Task identity、有序 Run attempt，以及互相独立的
  execution、verification 与 workspace outcome。
