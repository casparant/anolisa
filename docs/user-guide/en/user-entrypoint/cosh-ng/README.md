...
`cosh-gateway managed-run` and keep the remaining arguments unchanged.
...
- From another Terminal, set `gateway_socket` to the same absolute path and pipe
  ...
  The Managed Run surface exposes the same coordinator decisions as `start`,
  `answer`, `cancel`, `retry`, and `resolve-approval`, plus the evidence-aware
  `inspect` projection.
- [Managed Runs](managed-run.md) — inspect stable Task identity, ordered Run
  attempts, and separate execution, verification, and workspace outcomes.
