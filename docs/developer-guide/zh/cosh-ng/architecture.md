...
安装后的 `cosh` 启动器通常执行 `cosh-shell raw cosh-core`。`cosh-shell` 编译时
不依赖工作空间中的其他 crate，运行时则维护一个长时间存活的 cosh-core
子进程。两端都可能独立失败或重启，因此 stdin/stdout 协议需要保持向后兼容。
Contextual `/audit` 命令会跨越另一个有界进程边界，调用单一职责的
`cosh-audit` 工具；该工具属于内部实现，不是受支持的公开命令。
...
| `cosh-platform` | 内部 `cosh-audit` | 审计策略、存储、查询、导出和进程支持 | Gateway Task policy、Agent 交互或 checkpoint runtime 集成 |
...
- 为历史兼容保留的 ws-ckpt enum 顺序属于二进制 wire contract。cosh-ng 当前既不
  交付 checkpoint 命令，也不交付 runtime client；未来的 State/Recovery Provider 必须复用
  该 contract，或显式定义新版本。Task-only Gateway 不依赖 ws-ckpt daemon。
