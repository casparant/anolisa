...
cosh-ng separates the interactive terminal, Agent runtime, and Gateway Task
Plane so each boundary can be tested and integrated
...
cosh-shell /audit ---> cosh-audit ---> cosh-platform audit store
...
independently. The contextual `/audit` command crosses a separate bounded
process boundary to the single-purpose `cosh-audit` utility; the utility is an
internal implementation detail rather than a supported public command.

The Gateway addition keeps the existing Shell/Core paths intact. The
`cosh agent task` entrypoint is a local Unix control surface; it is not a Shell
slash-command surface and it does not open a network listener.
...
|---|---|---|---|
| `cosh-types` | — | Side-effect-free audit and compatibility wire types | OS access or runtime policy |
| `cosh-platform` | internal `cosh-audit` | Audit policy/store/query/export and process helpers | Gateway Task policy, Agent UX, or checkpoint runtime integration |
...
- Legacy ws-ckpt enum order remains a binary compatibility contract. cosh-ng
  currently ships neither a checkpoint command nor a runtime client; a future
  State/Recovery Provider must adopt or explicitly version that contract. The
  task-only Gateway does not depend on the ws-ckpt daemon.
