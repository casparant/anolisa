...
## Retained ws-ckpt compatibility protocol

`cosh-types` retains the historical ws-ckpt wire types so persisted fixtures
and downstream decoders do not change meaning during migration. COSH no longer
installs a checkpoint command or a ws-ckpt client. The types in
`crates/cosh-types/src/checkpoint.rs` describe the former Unix Domain Socket
payload: **bincode serialization with a 4-byte little-endian length prefix**.
