...
command = "/absolute/path/to/aw-cosh-hook --manifest /absolute/path/to/providers/tokenless/provider/provider.toml --executable-root /absolute/path/to/providers/tokenless/target/debug --target-id local-source-poc --allow-unenforced-provider --receipt-log /absolute/path/to/aw-receipts.jsonl"
timeout = 5000
sequential = true
```
