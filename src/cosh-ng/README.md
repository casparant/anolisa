...
`cosh-gateway managed-run` with the same remaining arguments.
...
cosh agent managed-run --socket "$gateway_socket" cancel '<tsk_UUID>' --run-id '<run_UUID>' \
...
The Managed Run decision surface maps those operations to `start`, `answer`,
`cancel`, `retry`, and `resolve-approval`. `managed-run inspect` reads the
complete authorized Task ledger in bounded pages
and presents one stable `TaskId` with its ordered `RunId` attempts. Runtime
success becomes `execution_completed`; independent verification and workspace
disposition remain explicitly `not_recorded` until authoritative durable facts
exist.
...
- [Inspect Managed Runs](../../docs/user-guide/en/user-entrypoint/cosh-ng/managed-run.md)
