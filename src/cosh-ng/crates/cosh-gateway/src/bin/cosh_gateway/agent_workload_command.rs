//! Agent Workload CLI projection over paged daemon Task events.

use cosh_gateway::agent_workload::AgentWorkloadProjector;

use super::*;

#[derive(Debug, Clone, Args)]
pub(super) struct WorkloadArgs {
    /// Absolute Unix socket path; defaults below the user runtime directory.
    #[arg(long, value_name = "PATH")]
    pub(super) socket: Option<PathBuf>,
    /// Presentation format for Agent Workload responses.
    #[arg(long, value_enum, default_value_t = Output::Human)]
    pub(super) output: Output,
    #[command(subcommand)]
    pub(super) command: WorkloadCommand,
}

#[derive(Debug, Clone, Subcommand)]
pub(super) enum WorkloadCommand {
...
pub(super) fn workload(args: WorkloadArgs, reporter: &Reporter) -> Result<u8, CliError> {
    let WorkloadArgs {
        socket,
        output,
        command,
    ...
    match command {
        WorkloadCommand::Start(command) => task(
            TaskArgs {
                socket,
                output,
        ...
        WorkloadCommand::Inspect(command) => inspect(socket, &command.task_id, reporter),
        WorkloadCommand::Answer(command) => task(
            TaskArgs {
                socket,
                output,
        ...
        WorkloadCommand::ResolveApproval(command) => task(
            TaskArgs {
                socket,
                output,
        ...
        WorkloadCommand::Cancel(command) => task(
            TaskArgs {
                socket,
                output,
        ...
        WorkloadCommand::Retry(command) => task(
            TaskArgs {
                socket,
                output,
...
fn inspect(socket: Option<PathBuf>, task_id: &str, reporter: &Reporter) -> Result<u8, CliError> {
    let task_id = parse_workload_task(task_id)?;
    ...
    loop {
        ...
        for event in &page.events {
            projector
                .apply(event)
                .map_err(|error| CliError::Workload(error.to_string()))?;
        }
    ...
    reporter.event(
        "agent_workload",
        serde_json::to_value(view).map_err(|error| CliError::Workload(error.to_string()))?,
    )?;
    Ok(0)
}

pub(super) fn parse_workload_task(value: &str) -> Result<TaskId, CliError> {
