...
fn target() -> TargetRef {
    TargetRef {
        ...
        identifier: BoundedOpaque::new("agent-workload-demo").unwrap(),
    }
}
...
fn provider_binding(task_id: &TaskId, run_id: &RunId) -> RuntimeBindingRef {
    RuntimeBindingRef {
        ...
        external_session: ExternalRef {
            ...
            authority: BoundedName::new("anthropic-managed-agents").unwrap(),
            scope_digest: Digest::parse("b".repeat(64)).unwrap(),
            value: BoundedOpaque::new("provider-session-42").unwrap(),
        },
    }
}
...
fn runtime_success_does_not_claim_verification_or_disposition() {
    ...
    let view = AgentWorkloadView::replay(&[
    ...
    assert_eq!(view.state, AgentWorkloadState::ExecutionCompleted);
    assert_eq!(view.completion.execution, WorkloadExecutionState::Succeeded);
...
#[test]
fn provider_session_is_correlated_without_becoming_workload_identity() {
    let task_id = TaskId::new();
    let actor_id = ActorId::new();
    let run_id = RunId::new();
    let binding = provider_binding(&task_id, &run_id);
    let view = AgentWorkloadView::replay(&[
        submitted(&task_id, &actor_id),
        event(
            &task_id,
    ...
    assert_eq!(view.task_id, task_id);
    assert_eq!(view.active_run_id.as_ref(), Some(&run_id));
    assert_eq!(view.attempts[0].runtime_binding.as_ref(), Some(&binding));
    assert_eq!(
        view.attempts[0]
            .runtime_binding
            .as_ref()
            .map(|runtime| runtime.external_session.kind),
        Some(ExternalRefKind::ProviderSession)
    );
}
...
fn uncertain_side_effect_suspends_without_inventing_a_result() {
    ...
    assert_eq!(view.state, AgentWorkloadState::Suspended);
    assert_eq!(view.completion.execution, WorkloadExecutionState::Uncertain);
...
fn incremental_projection_is_unchanged_after_invalid_event() {
    ...
    assert!(matches!(
        error,
        AgentWorkloadProjectionError::Aggregate(AggregateError::RevisionGap {
            expected: 2,
            actual: 3
        })
    ));
...
fn cancellation_before_runtime_keeps_the_attempt_and_stage() {
    ...
    assert_eq!(view.state, AgentWorkloadState::Cancelled);
    assert_eq!(view.completion.execution, WorkloadExecutionState::Cancelled);
...
fn suspension_and_known_execution_results_remain_distinct() {
    ...
    assert_eq!(view.state, AgentWorkloadState::Suspended);
    assert_eq!(view.completion.execution, WorkloadExecutionState::Suspended);
    assert_eq!(
