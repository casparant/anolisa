...
cosh-ng (Computable Operating System Harness) is the default interactive Agent environment for AweftOS. It provides the COSH shell, bundled Agent core, local task gateway, and a short-lived internal `cosh-audit` utility.
...
cargo test --package cosh-platform --test cosh_audit_cli  # Internal audit utility
...
6-crate workspace with three main runtime processes and one short-lived internal audit utility. Dependency direction: `cosh-core` → `cosh-platform` → `cosh-types`; `cosh-shell` is standalone (no internal crate deps). `cosh-gateway` depends only on the side-effect-free `cosh-gateway-contracts` leaf among internal crates.

- **cosh-types**: Pure types, zero side effects. Defines audit contracts, `CoshError`, and the retained ws-ckpt wire contract. COSH owns no ws-ckpt client; execution belongs to a future State/Recovery Provider.
- **cosh-platform**: Runtime support for audit persistence and process-group management. It also builds the single-purpose internal `cosh-audit` binary used by the bounded Shell `/audit` surface.
