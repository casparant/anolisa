...
normalize_modes() {
    ...
    chmod 0755 \
        ...
        "$stage/libexec/anolisa/cosh-ng/cosh-audit" \
...
stage_payload() {
    ...
    install -p -m 0755 "$BIN_DIR/cosh-audit" "$BIN_DIR/cosh-core" "$BIN_DIR/cosh-gateway" \
        "$BIN_DIR/cosh-shell" \
        "$stage/libexec/anolisa/cosh-ng/"
    install -p -m 0755 \
...
if [ -f "$BUILD_METADATA" ]; then
    ...
    python3 "$SCRIPT_DIR/verify-binaries.py" \
        ...
        "$BIN_DIR/cosh-audit" "$BIN_DIR/cosh-core" "$BIN_DIR/cosh-gateway" \
        "$BIN_DIR/cosh-shell"
