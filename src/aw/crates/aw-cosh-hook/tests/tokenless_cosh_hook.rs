...
fn cosh_post_tool_result_runs_through_core_and_tokenless() {
    ...
    let fixture: Value = serde_json::from_slice(
        &fs::read(
            root.join("providers/tokenless/provider/fixtures/context-projection-prepare.json"),
        )
    ...
    let run = run_cosh_post_tool_use(
        ...
        &CoshHookConfig {
            provider_source: ProviderManifestSource::File(
                root.join("providers/tokenless/provider/provider.toml"),
            ),
            provider_admission: ProviderAdmissionOptions {
                executable_roots: vec![root.join("providers/tokenless/target/debug")],
            },
