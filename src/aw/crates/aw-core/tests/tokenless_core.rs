...
fn core_prepares_a_real_tool_result_through_tokenless() {
    ...
    let catalog = ProviderCatalog::discover(
        ProviderManifestSource::File(package.join("provider/provider.toml")),
    ...
    let fixture: serde_json::Value = serde_json::from_slice(
        &fs::read(package.join("provider/fixtures/context-projection-prepare.json"))
