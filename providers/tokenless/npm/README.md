...
> channel. It does not install `provider/provider.toml` or its schemas into the
> host-wide discovery root. Use the raw, RPM, or `make install` distribution
> when Tokenless must be discoverable as an AW Provider Host.
