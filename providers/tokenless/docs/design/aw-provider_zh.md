...
[English](aw-provider.md)

Tokenless 把现有的一次性压缩协议作为 AW 的首个
`context.projection.prepare/v1` Provider 暴露出来。这个 Provider 包只是对
`tokenless compress` binary 的声明：Tokenless 仍然是独立程序，不依赖 COSH
或 Gateway crate。
...
机器可读的 JSON Schema 位于
[`provider/schemas/`](../../provider/schemas/)。manifest 使用通用的
...
源码目录把实现、manifest、schema 与 fixture 放在一起：
...
`make install`、RPM 和 raw package 会在 data prefix 下保持同样的 Provider
目录，并把 binary 安装到 binary prefix：
...
发现目录。因此，只安装 npm 包不会把 Tokenless 注册为 AW Provider Host。
