...
The Tokenless Provider package declares how AW uses Tokenless for context
projection. It exposes Tokenless's existing one-shot compression protocol as
the first `context.projection.prepare/v1` Provider for AW. The package contains
only the manifest, schemas, fixtures, and documentation; the independent
Tokenless implementation remains under [`src/tokenless/`](../../src/tokenless/).
...
The machine-readable JSON Schemas live in [`schemas/`](schemas/). The
manifest's generic
...
## Repository and installed layouts

The repository keeps implementation source and Provider declarations in two
explicit roots:

```text
src/tokenless/                 # implementation, tests, adapters, packaging
providers/tokenless/           # declarative Provider package
├── provider.toml
├── schemas/
├── fixtures/
├── README.md
...
This separation lets Tokenless evolve as a normal component while the Provider
package remains a small, reviewable declaration of how AW may invoke it.
Release packaging combines both roots: `make install`, RPM packages, and raw
packages install the executable under the binary prefix and the Provider
declarations under the canonical AW data directory:
...
npm alone does not register Tokenless with an AW Provider Host.
