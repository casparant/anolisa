...
fn main() -> Result<()> {
    ...
    // four levels below <repo>/providers/tokenless/benchmark/l2-module).
...
///
/// Scoped to both the repository-wide `src` tree and `providers/tokenless`:
/// the `rg_fn_main` sample scans `src/`, while cargo compiles whatever lands
/// under the Provider directory. `remote_sync.sh` copies untracked sources (it
/// excludes only build dirs), so either kind of file changes what was measured
/// while leaving `git_sha` and the tracked-dirty flag untouched; the count is
/// reported on its own so a non-zero value flags that the commit alone does not
